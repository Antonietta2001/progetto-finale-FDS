...
const MarketHeartbeat = () => {
  ...
  const calculateMetrics = () => {
    ...
    // ⭐ CORREZIONE 1: Calcola il Prezzo (WVAP) - Essenziale per il Backend
    ...
    // ⭐ CORREZIONE 2: Creazione del Punto Dati con il campo 'price'
    const newDataPoint = {
      time: new Date(timestamp).toLocaleTimeString(),
      ofi: parseFloat(ofi.toFixed(4)),
      spread_bps: parseFloat(spreadBps.toFixed(2)),
      // CAMPO ESSENZIALE PER IL BACKEND
        ...
        predictWithML(lstmSequenceRef.current)
            .then(mlResult => {
                setCurrentMetrics(prev => ({
                    ...
                    price: newDataPoint.price.toFixed(2), // Mostra il prezzo
  ...
  const predictWithML = async (sequence) => {
    try {
        ...
        if (!response.ok) {
            // Se il backend restituisce un errore 500, mostra il messaggio in console
